...
public class UserInterface {
	...
	public void start () {
		...
		Iterator<Citizen> iterator1 = linkedList.iterator();
		while (iterator1.hasNext()) {
         Citizen element = iterator1.next();
	...
	xmlsaver.Encode(linkedList, xmlpath);
		...
		if(file.exists()) {
			XMLSaver<CustomLinkedList<Citizen>> xmlsaver = new XMLSaver<>();
			linkedList.list = xmlsaver.Decode(xmlpath).list;
		} else {
		...
		if(file.exists()) {
			Serializator<CustomLinkedList<Citizen>> serializator = new Serializator<>();
			linkedList.list = serializator.Deseriaization(serpath).list;
		} else {
		    System.out.println("File does not exist.");
		}
	}
	...
	public void DataSorter (CustomLinkedList<Citizen> linkedList, Scanner sc)
	{
		...
		if (answer == 1) {
		linkedList.sortList(new CitizenNameComparator());
		}
		else if (answer == 2) {
		linkedList.sortList(new CitizenBirthDateComparator());		
		}
		else if (answer == 3) {
			linkedList.sortList(new CitizenLastinmateComparator());
		}
		else if (answer == 4) {
			linkedList.sortList(new CitizenLastFreeComparator());
		}
		else {
	...
	public static void main(String[] args) {
		...
		Linkedlist = xmlsaver.Decode(xmlpath);
